...
export default function WeatherDisplay({ data }: WeatherDisplayProps) {
  ...
  const stats = [
    ...
    { label: "Wind",        value: `${Math.round(current.windspeed)} km/h ${getWindDirection(current.winddirection)}` },
  ...
  return (
    <div className={styles.card}>
      <div className={styles.location}>
        ...
        <p className={styles.country}>{location.country} &middot; {data.timezone}</p>
      </div>

      <div className={styles.main}>
        <span className={styles.icon}>{weather.icon}</span>
        <div>
          <div className={styles.temp}>{formatTemp(current.temperature, unit)}</div>
